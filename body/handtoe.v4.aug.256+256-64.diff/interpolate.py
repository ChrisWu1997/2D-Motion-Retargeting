...
def interpolate(net, nr_sample, mode, form, device):
    ...
    def interpolate_as_form(a1, a2, c1):
        if form == 'line':
    ...
    if mode == 'motion':
        assert form == 'line'
        b_mix, m1 = interpolate_as_form(b1, b2, m1)
        dec_input = torch.cat([m1, b_mix.repeat(1, 1, m1.shape[-1])], dim=1)
        out12 = net.decoder(dec_input)

    elif mode == 'body':
        assert form == 'line'
        m_mix, b1 = interpolate_as_form(m1, m2, b1)
        dec_input = torch.cat([m_mix, b1.repeat(1, 1, m1.shape[-1])], dim=1)
        out12 = net.decoder(dec_input)

    elif mode == 'none':
        m_mix = vec_interpolate(m1, m2, alphas)
        b_mix = vec_interpolate(b1, b2, alphas)
        dec_input = torch.cat([m_mix, b_mix.repeat(1, 1, m1.shape[-1])], dim=1)
        out12 = net.decoder(dec_input)
...
if __name__=='__main__':
    ...
    parser.add_argument('--mode', type=str, choices=['motion', 'body', 'none'], default='none',
    ...
    if args.mode == 'body' or args.mode == 'motion':
