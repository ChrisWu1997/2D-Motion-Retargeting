...
def cluster_bodyview(net, cluster_data, device, save_path, is_draw=True):
    data, characters = cluster_data[0], cluster_data[2]
    data = data[:, :3, ::2, :, :].transpose(1, 2)
...
def cluster_motion(net, cluster_data, device, save_path, nr_anims=15, is_draw=True, mode='both'):
    ...
    if mode == 'body':
        ...
        data = data[:, 2, :, :, :].reshape(nr_anims, -1, data.shape[3], data.shape[4])
    else:
