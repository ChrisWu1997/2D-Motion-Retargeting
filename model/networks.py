...
def test1():
    mot_en_channels = [28 + 2, 64, 96, 128]
    body_en_channels = [28, 32, 48, 64]
    de_channels = [mot_en_channels[-1] + body_en_channels[-1], 128, 64, 28 + 2]
    net = AutoEncoder2x(mot_en_channels, body_en_channels, de_channels,
                      global_pool=F.max_pool1d, convpool=nn.MaxPool1d, compress=False)

    state_dict = torch.load("/home1/wurundi/code/2D-Motion-Retargeting/body/ablation.triplet-x.v3.128+64.weighted10x/model_state_dict.pth.tar")
    net.load_state_dict(state_dict)

    print(net)
    x = torch.ones((8, 30, 64))
    x = net(x)
    print(x.shape)
...
def test2():
    ...
    body_en_channels = [len_joints, 64, 96, 128, 32]
